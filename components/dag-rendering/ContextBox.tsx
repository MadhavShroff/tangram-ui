  ...
  return (
    <div className={`absolute top-4 right-4 bg-[#1E5A3E] shadow-md rounded-md transition-all duration-500 flex flex-col p-1 ${isCollapsed ? "w-32 h-10" : "w-72 max-h-80"}`}>
      <div className="flex justify-between">
        <button className="text-white p-2 bg-[#1E5A3E] hover:bg-[#353535] transition-colors rounded h-8 w-8" onClick={() => setIsCollapsed(!isCollapsed)}>
          {isCollapsed && <ChevronDown size={16} />}
        </button>
        <div className="text-[#c5ecd9] font-semibold text-lg mr-2 mt-[1.px]">Context</div>
      </div>
      {!isCollapsed && (
        <>
          <div className="relative flex-1">
            <ScrollArea className="h-20 w-full rounded-md min-h-0">
              {[...contextItems].reverse().map((item, idx) => (
                <div key={idx} className="flex items-center text-[#c5ecd9] text-sm px-2 py-1 bg-[#1E5A3E] rounded hover:bg-[#353535] transition-colors">
                  {getFileIcon(item)}
                  <span>{item}</span>
                </div>
              ))}
